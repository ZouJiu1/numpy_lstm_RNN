...
def train():
    ...
    for e in range(iters):
        ...
        hidden_lstm_in_r = [[] for i in range(num_layer)]
        hidden_col_in =[]
        hidden_col_in_r =[]
        loss_col = []
        delta_col = []
        ...
        inputs_col_r = [[] for i in range(num_layer)]
        middle_in_col = [[] for i in range(num_layer)]
        middle_in_col_r = [[] for i in range(num_layer)]
        hidden = []
        hidden_r = []
        cell = []
        cell_r = []
        ...
        for ind in range(num_layer):
            ...
            cell_r.append(np.zeros((batch_size, hidden_size[ind])))
        ...
        for j in range(sequence_length):
            for ind in range(num_layer):
                ...
                hidden_lstm_in_r[ind].append((hidden_r[ind], cell_r[ind]))
...
### reversed
            r_i = sequence_length - j - 1
            embedding_output = embedding.forward(inputs[r_i, :])
            flatten_col_r.append(embedding.flatten)
            hidden_cell_next, middle_in_col_0 = lstmlayers[0].forward(embedding_output, (hidden_r[0], cell_r[0]))
            hidden_r[0], cell_r[0] = hidden_cell_next
            hidden_cell_next, middle_in_col_1 = lstmlayers[1].forward(hidden_r[0], (hidden_r[1], cell_r[1]))
            hidden_r[1], cell_r[1] = hidden_cell_next

            hidden_col_in_r.append(hidden_r[1])
            middle_in_col_r[0].append(middle_in_col_0)
            middle_in_col_r[1].append(middle_in_col_1)
            inputs_col_r[0].append(embedding_output)
            inputs_col_r[1].append(hidden[0])

        # hidden_col_in_r.reverse()
        # middle_in_col_r.reverse()
        # flatten_col_r.reverse()
        # qzg_col[1].reverse()
        fullin = []
        for j in range(sequence_length):
            hidden_in = np.concatenate([hidden_col_in[j], hidden_col_in_r[sequence_length - j - 1]], axis = 1)
            fullin.append(hidden_in)
            y = fullconnect.forward(hidden_in)

            loss, delta, predict = cross_entropy_loss(y, labels[j, :, :])
            delta = qzg_col[j] * delta
            loss_col.append(loss)
            delta_col.append(delta)
            predict_col.append(predict)

        for j in np.arange(sequence_length):
            d_h_i1 = fullconnect.backward(delta_col[j], fullin[j])
            _i_, _f_, _g_, _o_, c_tanhk = middle_in_col_r[1][j]
            input_delta1, hidden_delta[1], cell_delta[1] = lstmlayers[1].backward(d_h_i1[:, hidden_size[1]:], hidden_delta[1], cell_delta[1], inputs_col_r[1][j], hidden_lstm_in_r[1][j], _i_, _f_, _g_, _o_, c_tanhk)
            _i_, _f_, _g_, _o_, c_tanhk = middle_in_col_r[0][j]
            input_delta0, hidden_delta[0], cell_delta[0] = lstmlayers[0].backward(input_delta1, hidden_delta[0], cell_delta[0], inputs_col_r[0][j], hidden_lstm_in_r[0][j], _i_, _f_, _g_, _o_, c_tanhk)
        ...
        for j in np.arange(sequence_length-1, -1, -1):
            ...
            input_delta1, hidden_delta[1], cell_delta[1] = lstmlayers[1].backward(d_h_i1[:, :hidden_size[1]], hidden_delta[1], cell_delta[1], inputs_col[1][j], hidden_lstm_in[1][j], _i_, _f_, _g_, _o_, c_tanhk)
        ...
        if e%showiter==0:
            ...
            for ind in range(num_layer):
                ...
                hidden_r.append(np.zeros((1, hidden_size[ind])))
            ...
            for j in range(sequence_length):
                ...
                hidden_col[0].append(hidden[1])

                embedding_output = embedding.forward(inputs[sequence_length - j - 1, -1])
                (hidden_r[0], cell_r[0]), middle_output = lstmlayers[0].forward(embedding_output, (hidden_r[0], cell_r[0]))
                (hidden_r[1], cell_r[1]), middle_output = lstmlayers[1].forward(hidden_r[0], (hidden_r[1], cell_r[1]))
                hidden_col[1].append(hidden_r[1])

            for j in range(sequence_length):
                hidden_in = np.concatenate([hidden_col[0][j], hidden_col[1][sequence_length - j - 1]], axis = 1)
                y = fullconnect.forward(hidden_in)
...
if __name__=="__main__":
    frequency = 1000
    pretrained_model = os.path.join(abspath, "models", 'tangshi_bidirectional_lstm2layerV2_embedding_%d.pkl'%frequency)
    train()