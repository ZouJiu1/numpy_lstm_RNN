...
def preprocess():
    jsonpth = os.path.join(abspath, 'dataset', r"tangshi_lstm3layer_dynamic.json")
...
def train():
    ...
    lstm_layer2 = lstmcell_layer(hidden_size[1], hidden_size[2], bias)
    lstmlayers = [lstm_layer0, lstm_layer1, lstm_layer2]
    
    lstm_decode0 = lstmcell_layer(embedding_dim, hidden_size[0], bias)
    lstm_decode1 = lstmcell_layer(hidden_size[0], hidden_size[1], bias)
    lstm_decode2 = lstmcell_layer(hidden_size[1], hidden_size[2], bias)
    lstm_decode = [lstm_decode0, lstm_decode1, lstm_decode2]
    ...
    if os.path.exists(pretrained_model):
        ...
        lstmlayers[2].restore_model(models[3])
        lstm_decode[0].restore_model(models[2*2])
        lstm_decode[1].restore_model(models[2*2+1])
        lstm_decode[2].restore_model(models[6])
    ...
    for e in range(iters):
        ...
        for i in range(len(all_content) - 1):
            ...
            for j in range(len(inputs)):
                ...
                hidden_cell_next, middle_in_col_2 = lstmlayers[2].forward(hidden[1], (hidden[2], cell[2]))
                hidden[2], cell[2] = hidden_cell_next
            ...
            for j in range(len(output)):
                ...
                hidden_cell_next, middle_output_2 = lstm_decode[2].forward(hidden_out[1], (hidden_out[2], cell_out[2]))
                hidden_out[2], cell_out[2] = hidden_cell_next
                ...
                middle_output_col[2].append(middle_output_2)
            ...
            for j in range(len(output) - 1, -1, -1):
                ...
                _i_, _f_, _g_, _o_, c_tanhk = middle_output_col[2][j]
                input_delta2, hidden_delta[2], cell_delta[2] = lstm_decode[2].backward(d_h_i1, hidden_delta[2], cell_delta[2], inputs_col_out[2][j], hidden_lstm_out[2][j], _i_, _f_, _g_, _o_, c_tanhk)
                _i_, _f_, _g_, _o_, c_tanhk = middle_output_col[1][j]
                input_delta1, hidden_delta[1], cell_delta[1] = lstm_decode[1].backward(input_delta2, hidden_delta[1], cell_delta[1], inputs_col_out[1][j], hidden_lstm_out[1][j], _i_, _f_, _g_, _o_, c_tanhk)
            ...
            for j in range(len(inputs)-1, -1, -1):
                _i_, _f_, _g_, _o_, c_tanhk = middle_in_col[2][j]
                input_delta2, hidden_delta[2], cell_delta[2] = lstmlayers[2].backward(zeros_delta, hidden_delta[2], cell_delta[2], inputs_colin[2][j], hidden_lstm_in[2][j], _i_, _f_, _g_, _o_, c_tanhk)
                _i_, _f_, _g_, _o_, c_tanhk = middle_in_col[1][j]
                input_delta1, hidden_delta[1], cell_delta[1] = lstmlayers[1].backward(input_delta2, hidden_delta[1], cell_delta[1], inputs_colin[1][j], hidden_lstm_in[1][j], _i_, _f_, _g_, _o_, c_tanhk)
            ...
            if e % showiter==0:
                ...
                for ij in range(len(inputs)):
                    ...
                    (hidden[2], cell[2]), middle_output = lstmlayers[2].forward(hidden[1], (hidden[2], cell[2]))
                ...
                for _ in range(len(output)):
                    ...
                    (hidden[2], cell[2]), middle_output = lstm_decode[2].forward(hidden[1], (hidden[2], cell[2]))
            ...
            if (e + 1) % savemodel == 0:
                ...
                model.append(e)
                with open(os.path.join(abspath, "models", 'tangshi_lstm3layer_embedding_dynamic_char.pkl'), 'wb') as obj:
...
if __name__=="__main__":
    pretrained_model = os.path.join(abspath, "models", 'tangshi_lstm3layer_embedding_dynamic_char.pkl')
    train()