...
def preprocess():
    jsonpth = os.path.join(abspath, 'dataset', r"tangshi_lstm3layerV2.json")
...
def train():
    ...
    for e in range(iters):
        ...
        for j in np.arange(sequence_length-1, -1, -1):
            ...
            input_delta2, hidden_delta[2], cell_delta[2] = lstmlayers[2].backward(d_h_i1, hidden_delta[2], cell_delta[2], inputs_col[2][j], hidden_lstm_in[2][j], _i_, _f_, _g_, _o_, c_tanhk)
            _i_, _f_, _g_, _o_, c_tanhk = middle_in_col[1][j]
            input_delta1, hidden_delta[1], cell_delta[1] = lstmlayers[1].backward(input_delta2, hidden_delta[1], cell_delta[1], inputs_col[1][j], hidden_lstm_in[1][j], _i_, _f_, _g_, _o_, c_tanhk)
        ...
        if (e + 1) % savemodel == 0:
            ...
            model.append(e)
            with open(os.path.join(abspath, "models", 'tangshi_lstm3layerV2_embedding_char.pkl'), 'wb') as obj:
...
if __name__=="__main__":
    pretrained_model = os.path.join(abspath, "models", 'tangshi_lstm3layerV2_embedding_char.pkl')
    train()