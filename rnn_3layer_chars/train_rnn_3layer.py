...
def preprocess():
    jsonpth = os.path.join(abspath, 'dataset', r"tangshi_rnn3layer.json")
...
def train():
    ...
    if os.path.exists(pretrained_model):
        ...
        rnn_layer0.restore_model(models[0])
        rnn_layer1.restore_model(models[1])
        rnn_layer2.restore_model(models[2])
    ...
    for e in range(start_iters, iters):
        ...
        labels = np.zeros((batch_size, input_size))
        ...
        if (e + 1) % savemodel == 0:
            ...
            model.append(e)
            with open(os.path.join(abspath, "models", 'tangshi_rnn3layer_char.pkl'), 'wb') as obj:
...
if __name__=="__main__":
    pretrained_model = os.path.join(abspath, "models", 'tangshi_rnn3layer_char.pkl')
    train()