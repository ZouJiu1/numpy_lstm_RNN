...
def preprocess():
    jsonpth = os.path.join(abspath, 'dataset', r"tangshi_rnn3layerV2.json")
    ...
    if not os.path.exists(jsonpth):
        with open(os.path.join(abspath, "dataset", r"tangshi_rnn3layerV2.json"), 'w') as obj:
...
def train():
    ...
    for e in range(iters):
        ...
        if (e + 1) % savemodel == 0:
            ...
            model.append(e)
            with open(os.path.join(abspath, "models", 'tangshi_rnn3layerV2_embedding_char.pkl'), 'wb') as obj:
...
if __name__=="__main__":
    pretrained_model = os.path.join(abspath, "models", 'tangshi_rnn3layerV2_embedding_char.pkl')
    train()