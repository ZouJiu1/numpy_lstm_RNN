...
def predict():
    with open(os.path.join(abspath, 'dataset', r"rnn1layer.json"), 'r') as obj:
    ...
    if os.path.exists(pth):
        ...
        exit(-1)
    rnn_layer.restore_model(models[0])
    ...
    for j in range(length):
        hidden_0 = rnn_layer.forward(first_inputs, hidden_0)
...
if __name__=="__main__":
    pth = os.path.join(abspath, "models", 'rnn1layer.pkl')
    predict()